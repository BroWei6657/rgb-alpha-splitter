...
RGB Alpha Splitter 源码采用 MIT License。第三方软件、SDK 和商标仍受其各自条款约束。

### NDI

NDI、NDI SDK 和 NDI Runtime 是 Vizrt 的产品或商标，不受本项目 MIT License
授权。本仓库不包含 NDI SDK 头文件、导入库或 NDI Runtime。开发者必须自行获取
SDK 并接受其许可证；最终用户必须自行安装兼容的 NDI Runtime。

RGB Alpha Splitter 是独立项目，与 Vizrt 不存在隶属、认可或赞助关系。

### Electron 与 Chromium

本应用使用 Electron 和 Chromium。其许可证声明由上游项目提供，并包含在打包后的
Electron 发行文件中。

### Node.js 软件包

开发依赖记录在 `package.json` 并由 `package-lock.json` 锁定。每个软件包继续受
其自身许可证约束。
...
not include NDI SDK headers, import libraries, or the NDI Runtime. Developers
must obtain the SDK separately and accept its license. End users must install
a compatible NDI Runtime separately.
